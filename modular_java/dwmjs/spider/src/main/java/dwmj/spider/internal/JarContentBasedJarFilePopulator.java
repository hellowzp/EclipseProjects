...
public class JarContentBasedJarFilePopulator implements JarFilePopulator {
   ...
   private void populateFromEntries(Enumeration<JarEntry> entries,
      ...
      Set<String> fullClassNames = new HashSet<String>();
      ...
      while (entries.hasMoreElements()) {
         ...
         if (fileName.endsWith(".class") && !fileName.contains("$")) {
            ...
            String fullClassName = fileName.replace(".class", "").replace('/', '.');
            fullClassNames.add(fullClassName);
            classNames.add(fullClassName.substring(fullClassName.lastIndexOf('.') + 1));
         }
      }
      jarFile.setPackages(packageNames);
      jarFile.setFullClassNames(fullClassNames);
      jarFile.setClassNames(classNames);
   }
