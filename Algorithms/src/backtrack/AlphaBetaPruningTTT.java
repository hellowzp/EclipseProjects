...
public class AlphaBetaPruningTTT {
	...
	private Map<Position, Integer> transpositions = new HashMap<>();
	private int[][] board = new int[3][3];

	// Constructor
	public AlphaBetaPruningTTT() {
		clearBoard();
	}
	...
	/**
	 * incorperate transposition table with alpha-beta pruning
	 * use HUMAN_WIN and COMPUTER_WIN as the initial values for
	 * alpha and beta respectively	
	 * 
	 * the recursion for a move stops when at least one possibility has been
	 * found that proves the move to be worse than a previously examined move,
	 * so as to prune away branches of the minimax tree that cannot influence
	 * the final decision
	 * 
	 * see Wikipedia for other extensions ==> Donald E. Knuth
	 */
	...
	// Find optimal move
	private Move chooseMove(int side, int alpha, int beta, int depth) {
		...
		Outer: for (int row = 0; row < 3; row++) {
			for (int column = 0; column < 3; column++) {
				if (squareIsEmpty(row, column)) {
					...
					if (side == COMPUTER && reply.val > value || side == HUMAN
							&& reply.val < value) {
						if (side == COMPUTER)
	...
	private int positionValue() {
		return isAWin(COMPUTER) ? COMPUTER_WIN : isAWin(HUMAN) ? HUMAN_WIN
