...
public class MediaPlayer extends JFrame {
	...
	public MediaPlayer(File file) {
		...
		super();
		this.file = file;
		fileDirection.add(file);
		saveListToFile(new File[]{file});
		this.setSize(width, height);
		this.setLocationRelativeTo(this);
		this.setLayout(new BorderLayout());
...
//			}
			...
			JOptionPane.showMessageDialog(this, "播放器目前不支持对此种格式的文件！\n"+e1.getMessage());
		...
		setTitle(fileName.elementAt(fileIndex).substring(
	...
	private void setAcceleratorInMenuItem() {
		...
		menuItem_allScreen.setAccelerator(KeyStroke.getKeyStroke("ENTER"));
	...
	private void initSystemTray() {
		...
		TrayIcon trayIcon = new TrayIcon(image, "媒体播放器", popupMenu);
