/***
 * 修改歌曲信息
 * 修改歌曲的界面与添加歌曲的界面完全一致，可以直接调用
 */
package musicServer.operateToDb;
...
public class UpdateMusic extends JFrame implements ActionListener{
	...
	public UpdateMusic(LinkDBModel linkDb,int rowNum)
	{
		...
		jtf_songname.setText((String)(linkDb.getValueAt(rowNum,0)));
		jtf_songname.setForeground(Color.magenta);
		jtf_songaddress=new JTextField(100);
		jtf_songaddress.setText((String)(linkDb.getValueAt(rowNum, 1)));
		jtf_singername=new JTextField(15);
		jtf_singername.setText((String)(linkDb.getValueAt(rowNum, 2)));
		...
		jcb_songList.setActionCommand("songList");
		String jcb_name=(String)(linkDb.getValueAt(rowNum, 3));
		//jcb_songList.setAction(getAppActionMap().get("ListAction"));
		if(jcb_name==null){
			jcb_name="默认列表";
		}
		jcb_songList.setSelectedItem(jcb_name);
		
		jtf_songlrc=new JTextField(100);
		jtf_songlrc.setText((String)(linkDb.getValueAt(rowNum, 4)));
		//设置组件位置
	...
	public void actionPerformed(ActionEvent e) {
		...
		//对复选框事件处理
		if(e.getActionCommand().equals("songList")){
		...
		if(e.getSource()==jb_sure)
		{
			...
			String songlist=this.getJcb_songList().getSelectedItem().toString();
			...
			if (songname == null || " ".equals(songname)) {
				...
				LinkDBModel linkDb=new LinkDBModel("musicInfo");
				linkDb.UpdateM(this);
				dispose();
