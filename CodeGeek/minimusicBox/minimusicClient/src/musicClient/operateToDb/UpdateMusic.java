...
public class UpdateMusic extends JFrame implements ActionListener{
	...
	//定义所需要的组件
	JPanel jp_add,jp1;
	JLabel jlb_title,jlb_songname,jlb_songaddress,jlb_singer,jlb_songlist,jlb_pic,jlb_songlrc;
	...
	public UpdateMusic(MusicModel mm,int rowNum)
	{
		...
		jtf_songname.setText((String)(mm.getValueAt(rowNum,0)));
		jtf_songname.setForeground(Color.magenta);
		jtf_songaddress=new JTextField(100);
		jtf_songaddress=new JTextField((String)(mm.getValueAt(rowNum, 1)));
		jtf_singername=new JTextField(100);
		jtf_singername.setText((String)(mm.getValueAt(rowNum, 2)));
		...
		//列表名
		String jcb_name=(String)(mm.getValueAt(rowNum, 3));
		//jcb_songList.setAction(getAppActionMap().get("ListAction"));
		jcb_songList.setName(jcb_name);
		//歌词位置
		jtf_songlrc=new JTextField(100);
		jtf_songlrc.setText((String)(mm.getValueAt(rowNum, 4))+"");
		//设置组件位置
		...
		//设置背景图片
		JPanel imagePanel;
		ImageIcon background=new ImageIcon("images/background8.jpg");
